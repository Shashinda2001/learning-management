...
import UserCourseProgress from "../models/userCourseProgressModel.ts";
import dotenv from "dotenv";

const { aws, Table, model: dynamooseModel } = pkg;
...
async function createTables() {
  ...
  for (const model of models) {
    ...
    const table = new Table(tableName, [model], {
      create: true,
      update: true,
      waitForActive: true,
...
async function seedData(tableName: string, filePath: string) {
  ...
  for (const item of data) {
    try {
      await dynamooseModel(formattedTableName).create(item);
    } catch (err) {
      console.error(
